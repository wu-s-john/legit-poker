...
import type { AnyTableSnapshot } from "./schemas/tableSnapshotSchema";
import { anyTableSnapshotSchema } from "./schemas/tableSnapshotSchema";
...
export const auth = {
  guest: () =>
    fetchApi<{ token: string; userId: string }>("/auth/guest", {
      method: "POST",
    }),
};
...
export const rooms = {
  ...
  join: (
    id: string,
    data: { role: "player" | "spectator"; pk_player?: string },
  ) =>
  ...
  start: (id: string) =>
    fetchApi<{ ok: boolean }>(`/rooms/${id}/start`, {
      method: "POST",
    }),
  transcript: (id: string, fromSeq: number, limit = 100) =>
    fetchApi<{ items: unknown[] }>(
      `/rooms/${id}/transcript?from_seq=${fromSeq}&limit=${limit}`,
    ),
};
...
export const actions = {
  post: (
    roomId: string,
    data: { idempotency_key: string; action: string; amount?: number },
  ) =>
...
export const console = {
  getHandSnapshot: async (
    gameId: string,
    handId: string,
  ): Promise<HandSnapshotWithMessages> => {
    const data = await fetchApi<{
      snapshot: unknown;
      messages?: FinalizedAnyMessageEnvelope[];
    }>(`/games/${gameId}/hands/${handId}/snapshot?include_messages=true`);

    // Parse the response
    const parsed = anyTableSnapshotSchema.parse(data.snapshot);
